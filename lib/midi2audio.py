...
class FluidSynth():
    def __init__(self, sound_font=DEFAULT_SOUND_FONT, sample_rate=DEFAULT_SAMPLE_RATE, fluidsynth_path='fluidsynth'):
        self.sample_rate = sample_rate
        self.sound_font = os.path.expanduser(sound_font)
        self.fluidsynth_path = fluidsynth_path  # 新增這行

    def midi_to_audio(self, midi_file, audio_file):
        subprocess.call([self.fluidsynth_path, '-ni', '-F', audio_file, '-g', '1.0', '-r', str(self.sample_rate), self.sound_font, midi_file])

    def play_midi(self, midi_file):
        subprocess.call([self.fluidsynth_path, '-g', '1.0', '-r', str(self.sample_rate), '-i', self.sound_font, midi_file])
